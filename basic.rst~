...
Groovyでは、以下のimportは暗黙的に定義されており、記述を省略できる。
...
Groovyは、基本的にJavaのソースコードをそのまま読み込ませても動作する。
...
Javaの場合、変数宣言時に限り、``Integer[] l = new Integer[] {1, 2, 3, 4}`` のような記述が
可能だが、角括弧による配列リテラルは、変数宣言以外でも記述可能。
...
Javaでは、例えば ``==`` のような比較演算子は、インスタンスが同一かどうかで判定を行うため、
プリミティブ型以外で値が同一かどうかを判定する場合はequalsメソッドを用いる必要があった。

Groovyにおいては、 ``==`` を使った比較では、内部的にequalsメソッドが呼び出されるため、
値が同一かどうかの判定となる。

また、不等号での比較の際も、内部でcompareToメソッドが呼び出される。
...
また、演算子は以下のようなメソッドに対応しており、対応するメソッドを定義しておけば、
...
戻り値として返される。ただし、 ``return`` は、Java同様、関数やメソッドの動作を途中で打ち切る
効果を持つ。
